...
```{mermaid}
sequenceDiagram
    participant M as Your machine<br/>(private key)
    participant G as GitHub<br/>(public key on file)
    M->>G: git push (signed with private key)
    G->>G: verify signature using stored public key
    G->>M: accepted, no password needed
```
